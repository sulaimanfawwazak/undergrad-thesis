...
class STTServer:
    def __init__(self, mode="mock", port=5556, model_size="base", 
                 device="cpu", vad_mode=1, sample_rate=16000, mic_device=None, energy_threshold=2.0, noise_alpha=0.98):
        """
        Args:
        ...
        self.noise_floor = 300.0        # initial RMS estimate
        self.energy_threshold = energy_threshold     # speech must be 2x louder than ambient
        self.noise_alpha = noise_alpha         # adaptation smoothing
    ...
    def calibrate_ambient_noise(self, duration=2.0):
        """
        Measure microphone ambient noise level.
        User should stay quiet during calibration.
        """

        print(
            f"{COLOR_CYAN}"
            f"Calibrating ambient noise ({duration}s)... stay quiet."
            f"{RESET}"
        )
        ...
        self.noise_floor = max(50.0, rms)

        print(
            f"{COLOR_GREEN}"
            f"Ambient calibrated:"
    ...
    def process_audio_chunk(self, chunk_bytes: bytes):
        """
        Adaptive Ambient Gate + WebRTC VAD
        """
        chunk_int16 = np.frombuffer(chunk_bytes, dtype=np.int16)
        chunk_float = chunk_int16.astype(np.float32)
        rms = np.sqrt(np.mean(chunk_float**2))

        # update ambient estimate ONLY while idle
        if not self.is_recording:
            self.noise_floor = (
                self.noise_alpha * self.noise_floor +
                (1.0 - self.noise_alpha) * rms
            )

        ambient_threshold = (
            self.noise_floor * self.energy_threshold
        )
        above_noise = rms > ambient_threshold
        vad_speech = self.vad.is_speech(
            chunk_bytes,
            self.sample_rate
        )
        is_speech = above_noise and vad_speech
        ...
        if is_speech:
            ...
            if self.silence_counter > self.silence_timeout_frames:
                ...
                text, confidence = self.transcribe_audio(
                    audio_float32
                )
...
def main():
    ...
    parser.add_argument("--energy-threshold", type=float, default=2.0)
    parser.add_argument("--noise-alpha", type=float, default=0.98)
