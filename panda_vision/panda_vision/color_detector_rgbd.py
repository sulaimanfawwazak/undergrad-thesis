...
class ColorDetector(Node):
    def __init__(self):
        super().__init__('color_detector_rgbd')
        ...
        for color_id, (lower, upper) in color_ranges.items():
            ...
            for cnt in contours:
                if cv2.contourArea(cnt) > 1:  # Increased minimum area threshold
                    ...
                    # Convert pixel -> camera frame (Inverse Projection)
                    # Forward Projection (FP) formula (3D -> Pixel):
                    # u = f_x * (X_c / Z_c) + c_x
                    # v = f_y * (Y_c / Z_c) + c_y
                    ...
                    try:
                        # Lookup transform rgbd_camera_link -> panda_link0 (Where is the camera relative to the robot's base)
